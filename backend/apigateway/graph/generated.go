...
func (ec *executionContext) _Subscription_color(ctx context.Context, field graphql.CollectedField) (ret func(ctx context.Context) graphql.Marshaler) {
	fc, err := ec.fieldContext_Subscription_color(ctx, field)
	if err != nil {
		return nil
	}
	ctx = graphql.WithFieldContext(ctx, fc)
	...
	resTmp, err := ec.ResolverMiddleware(ctx, func(rctx context.Context) (any, error) {
		ctx = rctx // use context from middleware stack in children
		return ec.resolvers.Subscription().Color(rctx)
	})
	if err != nil {
		ec.Error(ctx, err)
		return nil
	...
	return func(ctx context.Context) graphql.Marshaler {
		select {
		case res, ok := <-resTmp.(<-chan *model.Color):
			if !ok {
				return nil
			}
			return graphql.WriterFunc(func(w io.Writer) {
				...
				w.Write([]byte{':'})
				ec.marshalOColor2ᚖgithubᚗcomᚋ3thanHeadᚋapigatewayᚋgraphᚋmodelᚐColor(ctx, field.Selections, res).MarshalGQL(w)
				w.Write([]byte{'}'})
			})
		case <-ctx.Done():
			return nil
...
func (ec *executionContext) marshalOColor2ᚖgithubᚗcomᚋ3thanHeadᚋapigatewayᚋgraphᚋmodelᚐColor(ctx context.Context, sel ast.SelectionSet, v *model.Color) graphql.Marshaler {
	if v == nil {
		return graphql.Null
	}
